...
/* In 4:3 mode the game surface (640×480, native 4:3) is drawn into a
 * 320×240 centred window on the 400×240 top screen, leaving 40-pixel
 * black bars on each side.  The bar columns are zeroed once per
 * present call (see plat_video_present), not here, so this function
 * only has to fill the active window. */
#define TOP_43_W 320                        /* 240 * 4/3, exactly */
#define TOP_43_X ((TOP_WIDTH - TOP_43_W)/2) /* 40 px left bar     */
                            ...
                            uint16_t *fb, int src_w, int src_h)
{
    ...
    int use_43 = (g_aspect_mode[0] == '4');
    int dst_x0 = use_43 ? TOP_43_X : 0;
    int dst_w  = use_43 ? TOP_43_W : TOP_WIDTH;
    ...
    for (int dy = 0; dy < TOP_HEIGHT; ++dy) {
        ...
        /* Start at dst_x0 (0 in stretch mode, TOP_43_X in 4:3 mode).
         * The running-offset trick still applies: each step is
         * +PHYS_ROW_LEN regardless of which column we start at. */
        uint16_t *dst = fb + fb_pixel_index(dst_x0, dy);
        for (int dx = 0; dx < dst_w; ++dx) {
...
void platform_video_toggle_aspect_mode(void)
{
    ...
    g_aspect_mode[15] = '\0';
    s_have_prev_frame = 0; /* force full redraw next frame */
    LOG_INFO("3ds", "aspect_mode=%s", g_aspect_mode);
}
...
void plat_video_present(const uint8_t *shadow, const uint8_t *pal, int w, int h)
{
    ...
    if (top_dirty) {
        ...
        /* In 4:3 mode clear the side-bar columns (x < TOP_43_X and
         * x >= TOP_43_X + TOP_43_W) to black so no stale pixels leak
         * through when switching from stretch mode.  Each framebuffer
         * column is a contiguous run of PHYS_ROW_LEN u16 words (see
         * fb_pixel_index rationale at the top of this file), so we
         * zero them with a simple memset per column. */
        extern char g_aspect_mode[16];
        if (g_aspect_mode[0] == '4') {
            for (int bx = 0; bx < TOP_43_X; ++bx)
                memset(topfb + fb_pixel_index(bx, 0), 0,
                       PHYS_ROW_LEN * sizeof(uint16_t));
            for (int bx = TOP_43_X + TOP_43_W; bx < TOP_WIDTH; ++bx)
                memset(topfb + fb_pixel_index(bx, 0), 0,
                       PHYS_ROW_LEN * sizeof(uint16_t));
        }
